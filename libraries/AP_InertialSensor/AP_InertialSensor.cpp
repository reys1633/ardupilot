...
#define DEFAULT_GYRO_FILTER  0 //20
#define DEFAULT_ACCEL_FILTER 0 //20
#define DEFAULT_STILL_THRESH 2.5f
#elif APM_BUILD_TYPE(APM_BUILD_APMrover2)
#define DEFAULT_GYRO_FILTER  0 //4
#define DEFAULT_ACCEL_FILTER 0 //10
...
AP_InertialSensor::init(uint16_t sample_rate)
{
    ...
    hal.console->printf("Init Finished\n");
}
...
#define ADD_BACKEND(x) do { \
    ...
    case AP_BoardConfig::PX4_BOARD_PIXHAWK:
        ...
        ADD_BACKEND(AP_InertialSensor_LSM9DS0::probe(*this,
                                                      ...
                                                      ROTATION_PITCH_180));
        //ADD_BACKEND(AP_InertialSensor_LIS331::probe(*this, hal.i2c_mgr->get_device(HAL_INS_LIS331)));
        break;
...
{
    //hal.console->printf("Initializing _gyros...\n");
    ...
    */
    ...
    for(int8_t c = 0; c < 5; c++) {
        ...
        //hal.console->printf("Updating the gyros...\n");
        update();
    }
    //hal.console->printf("Gyros updated...\n");
    ...
    hal.console->printf("Finished _init_gyro\n");
}
...
void AP_InertialSensor::update(void)
{
    ...
    _have_sample = false;
    //hal.console->printf("Update finished\n");
}
...
void AP_InertialSensor::wait_for_sample(void)
{
    ...
    if (_next_sample_usec == 0 && _delta_time <= 0) {
        ...
        //hal.console->printf("Going to check_sample\n");
        goto check_sample;
    }
...
check_sample:
    //hal.console->printf("In check_sample\n");
    if (!_hil_mode) {
        //hal.console->printf("Check_sample\n");
        ...
        while (true) {
            for (uint8_t i=0; i<_backend_count; i++) {
                //hal.console->printf("Accumulating backends ");
            ...
            for (uint8_t i=0; i<INS_MAX_INSTANCES; i++) {
                ...
                //hal.console->printf("Sensors available ");
            }

            if (gyro_available && accel_available) {
                //hal.console->printf("Breaking.");
                break;
            }
            
            //hal.console->printf("Delaying");
