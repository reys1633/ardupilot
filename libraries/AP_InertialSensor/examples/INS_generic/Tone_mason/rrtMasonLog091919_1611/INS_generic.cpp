...
void loop(void)
{
    ...
    while( !stopex ) {      // loop forever
        if( time < tmax ) {
            ...
            time = AP_HAL::micros64()/1000000.0 - tStart;
            hal.console->printf("%8.3f %6d\n ", time, outcnt );
            ...
            for( int iins=0; iins<2; iins++) {
                ...
                for( int ichan=0; ichan<3; ichan++) {
                    ...
                    stor[idx] = gyr[ichan]; 
                    stor[idx+3] = acc[ichan];
                }
            } 
            outcnt += 12;
            stor[ outcnt ] = float( firesrm + firepara );
            outcnt++;
            /*
            // ignite SRM
...
//            hal.console->printf("    gx2     gy2     gz2     ax2     ay2     az2\n");
            ...
            int thisrow = 0;  // number printed on this row so far
