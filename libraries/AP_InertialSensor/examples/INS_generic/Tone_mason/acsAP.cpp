...
uint8_t acsAP::update( double ypCmd[2], double gyro[3], double rpy[3], double tof, double dtStep, int openLoop )
{
    // return;
    firesrm   = 0;
    firepara  = 0;
  ...
  //fyzCmd[0] = 0.0; // zero out y (yaw) command
    ...
    */
    ...
    //bool openloop = false;
    if( openLoop == 1 ) {
        float longperiod = 0.900;
        float shortperiod = 0.100;
        float t0 = 1.0; //2.0;
        ...
        if(tof>t0 && tof<=t1) pwmCmd[0] = 1; // 1.98 to 2
        if(tof>t2 && tof<=t3) pwmCmd[1] = 1; // 3.98 to 4
        if(tof>t4 && tof<=t5) pwmCmd[2] = 1; // 5.98 to 6
        if(tof>t6 && tof<=t7) pwmCmd[3] = 1; // 7.98 to 8
        //for(int i=0; i<4; i++) {
