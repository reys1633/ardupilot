...
#if USE_SD
    sdcard_init();
    fd = open( "/APM/test.dat", O_WRONLY|O_CREAT );
    write( fd, &anum, 4 );
    write( fd, &bnum, 4 );
    write( fd, &cnum, 4 );
    close( fd );
    
    fd = open( "/APM/test.dat", O_RDONLY );
    read( fd, &anum, 4 );
    hal.console->printf("anum = %d\n", (int)anum ); 
    read( fd, &bnum, 4 );
    hal.console->printf("bnum = %d\n", (int)bnum ); 
    read( fd, &cnum, 4 );
    hal.console->printf("cnum = %f\n", (float)cnum ); 
    close( fd );
    
    fd = open( "/APM/rrtlog.dat", O_WRONLY|O_CREAT );
    lfd = open( "/APM/rrt.dat", O_WRONLY|O_CREAT );
#endif
  //sdcard_stop();
