...
void loop(void)
{
 ...
 // double gyrmsr[3];
    ...
    double *dtLog  = new double[2500];       // Request memory for the variable
    ...
    while (!hal.console->available() ) {
        tStart = AP_HAL::micros() / 10000000.0;
    ...
    double tThis = AP_HAL::micros() / 1000000.0;
    dtLog[count] = tStart - tThis;
    
    // wait until scheduled time of completion
    while( tThis < tStart + dtStep ) {
        tThis = AP_HAL::micros() / 1000000.0;
    }
    
    if( tThis > 3.0 ) {
        for( int i=0; i< 250; i++ ) {
            for( int j=0; j< 10; j++ ) {
                hal.console->printf("%6d", (int)(dtLog[i*10+j]*1000000 ) );
            }
            hal.console->printf("\n");
        }
    }
