...
//      hal.console->printf( "time:%8.3f\n", time );      
         if( time > tmax ) {           // tof > tofmax ||
          //  ap.shutdown();
            ...
            // write out the data to console via mavlink
...
//			while( fd != EOF ) {
        ...
        for( int iins=0; iins<2; iins++) {
            OUT[count+iins,1] = gyro[0]; 
            OUT[count+iins,2] = gyro[1]; 
            OUT[count+iins,3] = gyro[2];
            OUT[count+iins,4] = accel[0]; 
            OUT[count+iins,5] = accel[1]; 
            OUT[count+iins,6] = accel[2];
        }
		    /*
