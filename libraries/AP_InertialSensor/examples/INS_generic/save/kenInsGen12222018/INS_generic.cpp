...
{           
    ...
    while (!hal.console->available() ) {
        ...
        if( trun <= 1.0L ) {
          trun = 1.0L; //hrt_absolute_time(); // when did INS start running
        }
        ...
        if( countPre++ < 999 || accel[2] < -19.6L ) {  // calibrate for a number of samples or a 2 gee pulse
          ...
          if ( false ) {
            ...
            hal.console->printf("%7d", (int)countPre );
          ...
          /////////////////////////////////////////////////////////////////
          if(countFly % 4 == 0) {
              ap.update( gyro, rpy, tnow );
          }
          /////////////////////////////////////////////////////////////////
 
          // motor ignition
          if(tnow > 0.050L && !motorIgnited ) {  // make sure that tnow starts at launch
               motorIgnited = true;
               //ap.ignMotor();
          }

          // parachute deployment
          if(tnow > 1.5L && !parachuteDep ) {
               parachuteDep = true;
               //ap.depParachute();
          }
          ...
          if (false) {
              hal.console->printf("%6.3f %6.3f %7.2f%7.2f%7.2f  %7.2f%7.2f%7.2f  %7.2f%7.2f%7.2f  %7.2f%7.2f%7.2f pwms:%3i\n", 
                     tnow, dt, accel[0], accel[1], accel[2], vel[0], vel[1], vel[2], pos[0], pos[1], pos[2],
