...
struct PACKED log_Test {
    LOG_PACKET_HEADER;
    float    time, time2;           // added time & time2
...
static const struct LogStructure log_structure[] = {
    ...
    { LOG_TEST_MSG, sizeof(log_Test),       
      "TEST",
      "ffHHHHii",
      "time,time2,V1,V2,V3,V4,L1,L2",             // added time
      "--------",
      "--------"
    }
};
...
void DataFlashTest::setup(void)
{
    ...
    for (i = 0; i < NUM_PACKETS; i++) {
        ...
        struct log_Test pkt = {
 	    ...
 	    time  : (float)(AP_HAL::micros()/1000000.0),
 	    time2  : (float)(AP_HAL::micros()/1000000.0),
            v1    : (uint16_t)(2000 + i),
            v2    : (uint16_t)(2001 + i),
            v3    : (uint16_t)(2002 + i),
            v4    : (uint16_t)(2003 + i),
        ...
        hal.scheduler->delay(20);
        int size = sizeof(struct log_Test);
        hal.console->printf("size: %d pkt: %d Avg write time %.1f usec/byte\n", 
                       size, i, (double)total_micros/( (double)i*size ) );
    }
